...
describe("fixed-origin image proxy", () => {
  ...
  it("maps an exact Cosmos thumbnail through the same local image boundary", () => {
    ...
    expect(remoteProductImage("/api/product-image/cosmos/7891000370100?__path=product-image%2Fcosmos%2F7891000370100"))
      .toBe("https://cdn-cosmos.bluesoft.com.br/products/7891000370100")
    expect(remoteProductImage("/api/product-image/cosmos/7891000370100?url=http://127.0.0.1"))
      .toBe("https://cdn-cosmos.bluesoft.com.br/products/7891000370100")
  })
  ...
  it.each(["/api/product-image?url=http://127.0.0.1", "/api/product-image/../../private", "/api/product-image/images/products/123/1.jpg", "/api/product-image/images/products/123/ingredients_pt.3.full.jpg"])("rejects unapproved destinations: %s", (path) => {
